...
def validate_query(
    ...
    try:
        ...
        try:
            ...
            canonical = ast_only.to_bcql()
        except Exception:
            ast_dict = EMPTY_AST
            canonical = ""
        return (
            _err_html("Query parses but does not match the corpus spec."),
    ...
    return (_ok_html(label), "", ast.model_dump(mode="json"), ast.to_bcql())
...
<div class="bcql-header">
  ...
  <p>Parse and validate <a href="https://blacklab.ivdnt.org/" target="_blank">BlackLab</a>
  Corpus Query Language queries with
  <a href="https://github.com/BramVanroy/bcql_py" target="_blank"><code>bcql_py</code></a>.</p>
</div>
"""
...
- **Round-trip** the parsed AST back to canonical BCQL.
    ...
    with gr.Blocks(
        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=5):
                ...
                gr.Examples(
                    examples=EXAMPLES,
                    inputs=[query_input, preset_dropdown],
                    label="Examples",
        ...
        gr.HTML(
            ...
            '<a href="https://github.com/BramVanroy/bcql_py">bcql_py</a> · '
        ...
        outputs = [status_box, error_md, ast_output, canonical_output]
        ...
        # Run once on page load so the default example is already validated.
        demo.load(validate_query, inputs=inputs, outputs=outputs)
